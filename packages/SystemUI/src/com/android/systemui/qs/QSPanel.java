...
public class QSPanel extends LinearLayout {
    ...
    protected void updateAutoBrightnessVisibility(int settingsVal) {
        if (mAutoBrightnessView != null) {
            mAutoBrightnessView.setVisibility(settingsVal != 0 &&
                    mIsAutomaticBrightnessAvailable ? View.VISIBLE : View.GONE);
        }
    }

    protected void updateBrightnessSliderPosition(int settingsVal) {
        mTop = settingsVal == 0;
    ...
    protected void updateBrightnessSliderVisibility(int settingsVal) {
        if (mBrightnessView != null) {
            mBrightnessView.setVisibility(settingsVal >= 1 ? VISIBLE : GONE);
        }
    }
